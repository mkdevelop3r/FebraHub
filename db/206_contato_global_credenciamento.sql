-- Contato global do Account do Salesforce passa a acompanhar o credenciamento.
begin;

alter table public.fato_credenciamento_turma
...
roster as (
  select distinct on (coalesce(f.cpf_norm, f.cliente_id, f.credenciamento_id), d.nome)
         ...
         coalesce(c.celular, nullif(f.telefone_cliente, ''), nullif(m.telefone_cliente, ''), a.telefone) as telefone,
         coalesce(c.email, nullif(f.email_cliente, ''), nullif(m.email_cliente, ''), a.email) as email,
...
-- Depois do proximo sync de credentialing, o CIS252 deve ficar sem lacunas
-- quando o Account correspondente possuir PersonMobilePhone/Phone ou PersonEmail.
